...
import { getMyBusinesses, type MyBusiness } from '@/lib/queries-panel';
...
export default function PanelPage() {
  ...
  useEffect(() => {
    ...
    if (user) {
      getMyBusinesses(user.id).then((data) => { setItems(data); setBizLoading(false); });
    }
  }, [loading, user, router]);
      ...
      <div className="ga-panel-wrap">
        ...
        {items.length === 0 ? (
          ...
          <div className="ga-mybiz-list">
            {items.map((b) => (
              <div key={b.id} className="ga-mybiz-card">
